...
var players = { }
var self_data = { name = '', position = Vector2(50, 50) }
...
remote func _send_player_info(id, info):
	players[id] = info
	var new_player = load('res://godot/src/players/Mage.tscn').instance()
	new_player.name = str(id)
	new_player.set_network_master(id)
	get_tree().current_scene.find_node("Objects").add_child(new_player)
